...
SRC_URI="https://github.com/k3s-io/k3s/archive/refs/tags/v1.28.5+k3s1.tar.gz -> ${P}.tar.gz
...
src_prepare() {
    ...
    local download_file="${S}/k3s-1.28.5-k3s1/scripts/download"
    local package_cli_file="${S}/k3s-1.28.5-k3s1/scripts/package-cli"
    local build_file="${S}/k3s-1.28.5-k3s1/scripts/build"

    # Check if the file exists before attempting to modify it
    if [ -e "${download_file}" ]; then
        echo "=== Contents of ${download_file} before sed ==="
        cat "${download_file}"
        ...
        echo "=== Contents of ${download_file} after sed ==="
        cat "${download_file}"
    else
        die "The file ${download_file} does not exist!"
    fi

    # Check if the file exists before attempting to modify it
    if [ -e "${package_cli_file}" ]; then
        echo "=== Contents of ${package_cli_file} before sed ==="
        cat "${package_cli_file}"

        # Modify scripts/package-cli to prevent external commands during the ebuild process
        sed -e 's:^[[:space:]]*curl:#\0:' \
            -e 's:^[[:space:]]*git:#\0:' \
            -e 's:^rm -rf \${CHARTS_DIR}:#\0:' \
            -e 's:^rm -rf \${RUNC_DIR}:#\0:' \
            -e 's:^rm -rf \${CONTAINERD_DIR}:#\0:' \
            -e 's:yq e :yq -r :' \
            -i "${package_cli_file}" || die

        echo "=== Contents of ${package_cli_file} after sed ==="
        cat "${package_cli_file}"
    else
        die "The file ${package_cli_file} does not exist!"
    fi

    # Check if the file exists before attempting to modify it
    if [ -e "${build_file}" ]; then
        echo "=== Contents of ${build_file} before sed ==="
        cat "${build_file}"

        # Modify scripts/build to prevent external commands during the ebuild process
        sed -e 's:^[[:space:]]*curl:#\0:' \
            -e 's:^[[:space:]]*git:#\0:' \
            -e 's:^rm -rf \${CHARTS_DIR}:#\0:' \
            -e 's:^rm -rf \${RUNC_DIR}:#\0:' \
        ...
        echo "=== Contents of ${build_file} after sed ==="
        cat "${build_file}"
    else
        die "The file ${build_file} does not exist!"
    fi

    pattern='git clone -b $VERSION_CNIPLUGINS https://github.com/rancher/plugins.git $WORKDIR'
	filename="${S}/k3s-1.28.5-k3s1/scripts/build"
    ...
    sed -e "s|${pattern}|mkdir -p \"\$WORKDIR\" \\&\\& tar -xzf \"${S}/cache/k3s-cni-plugins-${K3S_CNIPLUGINS_VERSION}.tar.gz\" --strip-components=1 -C \"\$WORKDIR\"|" \
        -e 's|rm -rf $TMPDIR||' \
        -i "${filename}" || die
		sed -e 's:/usr/local/bin:/usr/bin:g' -i "${S}/k3s-1.28.5-k3s1/k3s.service" || die
}

src_compile() {
    mkdir -p build/data || die
    
    local download_script="${S}/k3s-1.28.5-k3s1/scripts/download"
    local build_script="${S}/k3s-1.28.5-k3s1/scripts/build"

    if [ -e "${download_script}" ]; then
        echo "=== Contents of ${download_script} ==="
        cat "${download_script}"

        "${BASH}" -ex "${download_script}" || die
    else
        die "The file ${download_script} does not exist!"
    fi

    if [ -e "${build_script}" ]; then
        echo "=== Contents of ${build_script} ==="
        cat "${build_script}"

        "${BASH}" -ex "${build_script}" || die
    else
        die "The file ${build_script} does not exist!"
    fi
...
src_install() {
    ...
    newbin "${S}/k3s-1.28.5-k3s1/k3s-killall.sh" k3s-killall
    use kubectl-symlink && dosym k3s /usr/bin/kubectl
    systemd_dounit "${S}/k3s-1.28.5-k3s1/k3s.service"
    newinitd "${S}/k3s-1.28.5-k3s1/k3s.initd" "${PN}"
    newconfd "${S}/k3s-1.28.5-k3s1/k3s.confd" "${PN}"
    insinto /etc/logrotate.d
    newins "${S}/k3s-1.28.5-k3s1/k3s.logrotated" "${PN}"
    dodoc README.md
}
